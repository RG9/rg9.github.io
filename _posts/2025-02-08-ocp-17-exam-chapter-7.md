---
title: OCP 17 Exam - chapter 7 notes
...
*  tricky difference between `interface` and `abstact class` is that **subtype** of `interface` must always provide `public` modifier for implemented method
```java
...
- class inherits from **two or more interfaces with default methods with the same signature**
	...
	- when we want to use/call `default` method implementation from Interface, we should use `InterfaceName.super.methodName();` syntax.
 ```java
...
<https://github.com/RG9/rg-playground-ocp17/blob/main/Chapter7.java>
