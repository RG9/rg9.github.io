...
- Java provides JDBC interfaces, while database libraries provides implementations - except implementation of `DriverManager`, which is used to create connection (Q.1)
...
- **rollback** will throw if you try to rollback to already rollbacked savepoint (after savepoint used to rollback), so order matters. Also you cannot rollback to the same savepoint twice (Q.15)
