...
```java
    ...
    List<String> strings1 = new ArrayList();  // ok
    
    // List˂String˃ strings = new ArrayList˂?˃(); DOES NOT COMPILE 
    
     // List<> list = new ArrayList<String>(); // error: illegal start of type

     List<?> strings2 = new ArrayList<String>(); // ok
     
     List strings3 = new ArrayList<String>(); // ok

    List<Object> objects = new ArrayList(); // warn:  uses unchecked or unsafe operation
    
    var obj3 = new ArrayList(); // ok
...
- what is valid method override with a collection as parameter and return type (review Q.7) - we can omit `<>` for both return type and parameter, besides return type can be covariant e.g. `ArrayList`:
...
```java
  ...
  static class BarOverride extends Foo {
    ...
    // ArrayList<String> bar(List<String> a) {  System.out.println("BarOverride"); return null; } // ok (covariant return type)
...
- be aware of reverse order by comparing second parameter of `Comparator#compareTo` to first (review Q.8)
...
```java
    ...
    System.out.println(collection);
    collection.retainAll(List.of(4, 5, 6)); // remove all elements that are not in the collection
    System.out.println(collection);
...
```java
    ...
    System.out.println(queue.element()); // in contrary to peek() throws exception when queue is empty
    // --
