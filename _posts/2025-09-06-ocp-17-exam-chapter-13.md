...
- `tryLock(long timeout, TimeUnit unit)` works that waits specified amount of time for lock, but returns `false` instead throwing timeout exception

## Runnable vs Callable

- `Callable` could throw checked Exception, but `Runnable` does not (Q.3)
