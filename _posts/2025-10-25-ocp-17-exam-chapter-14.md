...
- **byte** streams are called `*InputStream`/`*OutputStream` (e.g. `FileInputStream`, `BufferedOutputStream`), where **character** streams are `*Reader`/`*Writer` (e.g. `FileReader`, `BufferedWriter`)

- exception is `PrintStream` (extend `OutputStream` but has character methods e.g. `System.out.println`)
